...
async def event_ready():
    print("Your bot is ready little skidder")
...
async def StartFNClient():
    ...
    try:
        ...
        return response.json({"error":"Codice di autorizzazzione errato"})
    except Exception as e:
        return response.json({"error":"SexyMessage error","errorMessage":e})
...
async def Authenticate(request):
    ...
    if request.headers.get('X-Secret-Password'):
        if ClientSettings.HeaderPassword:
            ...
            return response.json({"error":"Nessuna password è stata settata"},status=200)
    else:
        return response.json({"error":"Fuck u skid"},status=400)

@app.route('/')
async def Home(request):
    return response.text('Follow @lucifer..ff on TikTok')
...
async def Update(request):
    ...
    NewSettings = requests.get('Settings.json').json()
    for Key,Value in NewSettings.items():
        if Key in CurrentSettings and Key != "Bot Version":
            NewSettings[Key] = CurrentSettings[Key]

    z = zipfile.ZipFile(io.BytesIO((requests.get('Give a fuck about u palbot')).content))
...
async def settings(request):
    ...
    if request.body:
        try:
            ...
            return response.json({"error":"Sexy error"})
...
async def start(request):
    ...
    if fnClient.is_ready():
        return response.json({"error":"I'm to hot for this"},status=200)
...
async def stop(request):
    ...
    for client in fnClient.Clients.values():
        await client.logout()
    await fnClient.logout()
    return response.json({"success":"FortniteBalls!"})

#Start Server
loop = asyncio.get_event_loop()
loop.create_task(app.create_server(host="127.0.0.1", port=6969, return_asyncio_server=True)) //storm fn port do not use
